# MGBX 合约 WebSocket API 接入文档

## 1. WebSocket 概述

> ⚠️ **重要提示**：合约交易功能需要联系客服开通后方可使用。请通过平台官网联系在线客服或发送邮件至客服邮箱申请开通合约交易权限，开通后即可使用以下 WebSocket 接口进行合约实时数据订阅。

MGBX 交易平台提供合约 WebSocket 接口，支持实时订阅行情数据和用户数据，相比 REST API 具有更低的延迟和更高的效率。WebSocket 连接不需要认证即可订阅市场数据，但订阅用户私有数据时需要提供认证 listenKey。

## 2. 服务地址

- 市场数据WebSocket基础URL: `wss://open.mgbx.com/fut/v1/ws/market`
- 用户数据WebSocket基础URL: `wss://open.mgbx.com/fut/v1/ws/user`
- 获取用户listenKey URL: `https://open.mgbx.com/fut/v1/user/listen-key`
...
- `coin`: 币种
- `balanceType`: 账户类型
- `underlyingType`: 标的类型（1=币本位，2=U本位）
- `walletBalance`: 钱包余额
...
- `bounsMargin`: 奖励保证金
- `openOrderMarginFrozen`: 委托冻结保证金
- `underlyingType`: 标的类型（U_BASED=U本位，COIN_BASED=币本位）
