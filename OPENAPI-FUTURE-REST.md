# MGBX 合约 OpenAPI 接入文档

## 1. API 概述

> ⚠️ **重要提示**：合约交易功能需要联系客服开通后方可使用。请通过平台官网联系在线客服或发送邮件至客服邮箱申请开通合约交易权限，开通后即可使用以下 API 接口进行合约交易。

MGBX 合约交易平台 API 提供了程序化交易的能力，允许开发者通过 HTTP 请求创建订单、查询市场数据等功能。
...
|-------|-----|------|
| id | Long | 交易对ID |
| symbol | String | 交易对名称，例如 `btc_usdt` |
| contractType | String | 合约类型，`PERPETUAL`=永续合约 |
| underlyingType | String | 标的类型，`U_BASED`=U本位，`COIN_BASED`=币本位 |
| contractSize | String | 合约乘数/面值（每张合约代表的基础资产数量） |
| tradeSwitch | Boolean | 是否开启交易，`true`=可交易，`false`=暂停交易 |
| state | Integer | 交易对状态，`0`=正常 |
| initLeverage | Integer | 初始杠杆倍数（最大可用杠杆） |
| initPositionType | String | 初始持仓模式，`CROSSED`=全仓，`ISOLATED`=逐仓 |
| baseCoin | String | 基础资产，例如 `btc` |
| quoteCoin | String | 计价资产，例如 `usdt` |
| baseCoinPrecision | Integer | 基础资产精度 |
| quoteCoinPrecision | Integer | 计价资产精度 |
| quantityPrecision | Integer | 数量精度（下单数量的小数位数） |
| pricePrecision | Integer | 价格精度（下单价格的小数位数） |
| supportOrderType | String | 支持的订单类型，逗号分隔，如 `LIMIT,MARKET` |
| supportTimeInForce | String | 支持的有效期类型，逗号分隔，如 `GTC,FOK,IOC,GTX` |
| supportEntrustType | String | 支持的委托类型，逗号分隔，如 `TAKE_PROFIT,STOP,TAKE_PROFIT_MARKET,STOP_MARKET,TRAILING_STOP_MARKET` |
| supportPositionType | String | 支持的持仓模式，逗号分隔，如 `CROSSED,ISOLATED` |
| minPrice | String | 最小下单价格，可为 `null` 表示无限制 |
| minQty | String | 最小委托数量（张） |
| minNotional | String | 最小委托金额（USDT） |
| maxNotional | String | 最大委托金额（USDT） |
| multiplierDown | String | 限价卖单价格下限百分比 |
| multiplierUp | String | 限价买单价格上限百分比 |
| maxOpenOrders | Integer | 最大挂单数量 |
| maxEntrusts | Integer | 最大条件委托数量（止盈止损等） |
| makerFee | String | 挂单（Maker）手续费率 |
| takerFee | String | 吃单（Taker）手续费率 |
| liquidationFee | String | 强平手续费率（用于强平清算） |
| marketTakeBound | String | 市价单价格保护偏差比例 |
| depthPrecisionMerge | Integer | 深度数据精度合并级别 |
| labels | Array[String] | 标签列表，如 `["HOT"]` |
| onboardDate | Long | 上线日期时间戳（毫秒） |
| enName | String | 英文名称 |
| cnName | String | 中文名称 |
...
|-------|-----|-----|------|
| symbol | String | 是 | 交易对，例如："BTC_USDT" |
| num | Integer | 否 | 获取数量，最小值为1，默认值 50  |
...
| timeInForce | String | 否 | 有效方式，限价单支持：`GTC`、`IOC`、`FOK`、`GTX`； 限价单默认 `GTC` |
...
| state | String | 订单状态 |
| timeInForce | String | 有效方式，限价单支持：`GTC`、`IOC`、`FOK`、`GTX` |
...
|-------|-----|---------|------|
| symbol | String | 否 | 交易对 |
| state | String | 否 | 订单状态筛选：NEW(新建)、FILLED(全部成交)、CANCELED(已撤销) |
...
> ⚠️ **注意**：此接口为异步接口，调用成功仅表示撤单请求已提交，实际撤单结果需通过**订单查询接口**（[5.9 查询订单详情](#59-查询订单详情) 或 [5.10 查询订单列表](#510-查询订单列表)）确认订单状态是否已变更为 `CANCELED`。
...
| code | Integer | 状态码，0表示成功 |
| msg | String | 响应信息 |
| data | Object | 撤单结果, 撤单成功返回 orderId |
...
| triggerStopPrice | String | 止损触发价，可能为 null |
| triggerPriceSide | Integer | 触发方向,可能为 null |
| entryPrice | String | 开仓均价 |
| positionSize | String | 持仓数量 |
| isolatedMargin | String | 逐仓保证金 |
| triggerPrice | String | 当前触发价格 |
| profitType | String | 止盈止损类型 `NORMAL`(普通止盈止损)、`ALL`(移动止盈止损) |
